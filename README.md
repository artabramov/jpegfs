...
`check` — display container information (`UUID`, `generation`, `threshold`, total shard count, container size) and verify container integrity. Checks whether enough shards are available, whether the container can be reconstructed and decrypted, and whether the ZIP archive can be read successfully.
...
`ls`, `list` — list files stored in the container.
